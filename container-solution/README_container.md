...
Container for Falcon using SSH login inside container to submit sbatch script. This is an alternative way to the conda solution, if a container approach is favoured. Please note the below instructions is in the case that the `README_container.md`, `falcon-container.sh` and `sbatch_container.sh` in this directory replace `README.md`, `falcon-conda.sh` and `sbatch_nextflow.sh` in the main directory. 
